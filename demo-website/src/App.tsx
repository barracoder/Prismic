  ...
  useEffect(() => {
    ...
    if (welcomeRegionRef.current && contentRegionRef.current) {
      try {
        ...
        // Add components to the regions directly
        if (!welcomeRegion.getComponents().some(c => c.id === 'welcome-1')) {
          welcomeRegion.addComponent({
        ...
        if (!contentRegion.getComponents().some(c => c.id === 'counter-1')) {
          contentRegion.addComponent({
